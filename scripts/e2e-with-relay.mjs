...
function normalizeMultiaddrForBrowserDial(ma) {
  ...
  return ma.replace(/\/ip4\/0\.0\.0\.0\//g, '/ip4/127.0.0.1/').replace(/\/ip6\/::\//g, '/ip6/::1/');
}

function pickWebsocketBootstrap(payload) {
  const wsList = Array.isArray(payload.byTransport?.websocket) ? payload.byTransport.websocket : [];
  const candidates = [
...
console.log(
  '[e2e] Relay PID',
  relayProcess.pid,
  '— bootstrap (loopback WS for browser):',
  bootstrap
);
