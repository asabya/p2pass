...
export async function gotoExample(page) {
	await page.goto('/');
	await expect(page.getByTestId('storacha-fab-button')).toBeVisible({ timeout: 30000 });
	await waitForWidgetApi(page);
}

export async function openWidget(page) {
	await gotoExample(page);
	await page.getByTestId('storacha-fab-button').click();
	await expect(page.getByTestId('storacha-widget-panel')).toBeVisible();
}
