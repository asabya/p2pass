...
test.describe('P2Pass Multi-Device E2E', () => {
  ...
  test('shows a pairing request prompt and adds the second device after approval', async ({
    browser,
  }) => {
    const contextA = await createContextWithMock(browser, SEED_A);
    const contextB = await createContextWithMock(browser, SEED_B);
    ...
    try {
      ...
      await pageA.evaluate(
        (payload) => window.__p2pPasskeysWidget.beginIncomingPairingRequest(payload),
        request
      );
  ...
  test('auto-grants known devices without reopening the approval prompt', async ({ browser }) => {
    ...
    try {
      ...
      const firstGrant = await pageA.evaluate(
        (payload) => window.__p2pPasskeysWidget.simulateIncomingPairingRequest(payload, 'granted'),
        request
      );
      expect(firstGrant.type).toBe('granted');

      const secondGrant = await pageA.evaluate(
        (payload) => window.__p2pPasskeysWidget.simulateIncomingPairingRequest(payload, 'rejected'),
        request
      );
